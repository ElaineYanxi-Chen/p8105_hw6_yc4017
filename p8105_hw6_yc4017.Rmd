...
Of course there are other potential factors from this dataset that could affect birthweight, but I do not want to include too many variables due to concerns about sample size.
...
Looking at the violin plots, we see that the model containing three-way interactions between head circumference, length at birth, and sex of the baby has the lowest RMSEs, which suggests that it has the best performance. The model I built had the worst performance, probably because it failed to acocunt for interactions between some of the variables in the model. 